...
namespace ShadowWinForms
{
    public class ShaderLoader
    {
        ...
        {
            shaderSource = File.ReadAllText(shaderFileName);
        }
        ...
        public static int CreateProgram(string vShader, string fShader)
        {
            ...
            if (status == 0)
            {
                string errorString = $"Failed to link program: {GL.GetProgramInfoLog(program)}\n";
        ...
        private static int LoadShader(ShaderType shaderType, string shaderSource)
        {
            ...
            if (status == 0)
            {
                string errorString = $"Failed to compile {shaderType} shader: {GL.GetShaderInfoLog(shader)}";
                Logger.Append(errorString);
